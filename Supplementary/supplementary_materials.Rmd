...
```{r fig5b, fig.height=6, fig.width=7, fig.cap="**Spread of interactive effects across threats and systems.**"}
...
```{r figwaffle, fig.height=6, fig.width=5, messages = FALSE,fig.cap="**Demonstration of how changing the credible interval from 50 to 95% alters the classification of each threat combination.**"}

post_interval_intvadd_diffW <- post_intvadd_diff %>%
  na.omit() %>%
  group_by(threat_group, threats) %>%
  ggdist::median_qi(.width = c(.95, .5), .exclude = c(".draw")) %>%
  mutate(
    interaction.type = factor(case_when(
      .upper < 0 ~ "synergistic",
      .lower > 0 ~ "antagonistic",
...
data_adW <- post_interval_intvadd_diffW %>%
  separate_rows(threats, sep = "\\.") %>%
  mutate(interaction.type = factor(
    interaction.type,
    levels = c("synergistic", "additive", "antagonistic")
  )) %>%
  group_by(threats, interaction.type,.width) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  complete(threats, interaction.type,.width) %>%
  group_by(threats,.width) %>%
  mutate(n = replace_na(n, 0), freq = (n / sum(n))) %>%
  ungroup()
...
legendW <- cowplot::get_plot_component(
  dataW %>%
...
gWa <- ggplot(dataW, aes(fill = interaction.type, values = freq)) +
    geom_waffle(
      color = "white",
      size = .25,
      n_rows = 10,
...
gWa / legendW + plot_layout(heights = c(1, .1))

```

\newpage

```{r figS6a, fig.height=12, fig.width=14, messages = FALSE,fig.cap="**Counterfactual predictions for multiple threats.** The counterfactuals represent the changes in the population trends of the 1,740 vertebrate time-series affected by threats, had there been different combinations of multiple threats. (A) Counterfactuals representing the global vertebrate population growth where different combinations of two threats were removed. (B) Counterfactuals representing the global vertebrate population growth where three threats were removed. The black line represents when the population trend is 0. The dotted line is the median trend of the threatened populations without any intervention."}
...
```{r figShaversine, fig.height=4, fig.width=6, messages = FALSE,fig.cap="**Influence of rounding latitude to the nearest degree upon the estimated distance between Living Planet Database sites.**"}
dd_long2 <- dd_long %>%
  group_by(ID) %>%  
...
mat_raw <- geosphere::distm(distinct(mod_dat_full_unrounded[,c("Longitude","Latitude")]), 
                        fun = geosphere::distHaversine)
diag(mat_raw) <- NA #want to ignore distance to self
mat_raw <- mat_raw |>
  as.data.frame() |>
  tidyr::pivot_longer(dplyr::everything(),names_to = NULL,values_to = "dist") |>
  dplyr::mutate(dist = dist/1000) |> 
  na.omit()  |>
  dplyr::mutate(datasource = "Unrounded")

mat_rounded <- geosphere::distm(distinct(mod_dat_full[,c("Longitude","Latitude")]), 
                        fun = geosphere::distHaversine)
diag(mat_rounded) <- NA
mat_rounded <- mat_rounded |>
  as.data.frame() |>
  tidyr::pivot_longer(dplyr::everything(),names_to = NULL,values_to = "dist") |>
  dplyr::mutate(dist = dist/1000) |> 
  na.omit() |>
  dplyr::mutate(datasource = "Rounded")

raw_ecdf <- ecdf(mat_raw$dist)
rounded_ecdf <- ecdf(mat_rounded$dist)
label_df <- data.frame(datasource = c("Unrounded", "Rounded"),
                       x = 500, 
                       y = 2e-5,
                       quantile55 = c(raw_ecdf(55),rounded_ecdf(55))) |>
  dplyr::mutate(label = paste0("55km =\n", round(quantile55,digits = 6)," percentile"))

p_dist <- ggplot(rbind(mat_raw, mat_rounded) |>
         dplyr::mutate(datasource = factor(datasource, levels = c("Unrounded","Rounded"))),
       aes(x = dist)) + geom_density() +
  geom_vline(xintercept = 55, color = "#1E63B3", linetype = "dashed") + 
  geom_label(data = label_df,aes(x = x, y = y, label = label), color = "#1E63B3", hjust = 0) +
  #annotate(geom= "label", x = 500, y = 9e-5,label = "55km", color = "#1E63B3") +
  xlab("Distance between sites (km2)") +
  ylab("Density") +
...
```{r figS2a, cache = FALSE, autodep=FALSE, fig.height=8, fig.width=12, fig.cap="**Distribution of the standardised residuals for the multilevel Bayesian models.** Residuals were examined for the (A) global model, (B-D) system specific models (freshwater, marine, terrestrial), and (E-I) amphibians, birds, fishes, mammals, and reptiles respectively."}
...
```{r figS2c, cache = FALSE, autodep=FALSE, fig.height=8, fig.width=10, fig.cap="**The posterior predictive checks for the multilevel Bayesian models.** The posterior predictive checks do not show strong discrepancies between our data (dark lines, y) from the predictions from the model (light grey lines, yrep) for any of the models. However, the model shows a slight underestimation of the true zero values. The posterior predictive checks were examined for the (A) global model, (B-D) system specific models (freshwater, marine, terrestrial), and (E-I) amphibians, birds, fishes, mammals, and reptiles respectively."}
...
```{r figS2e, cache = FALSE, autodep=FALSE, fig.height=6, fig.width=10, fig.cap="**Autocorrelation of the standardised residuals for the multilevel Bayesian models.** Limited evidence for autocorrelation is present for the (A) global model, (B-D) system specific models (freshwater, marine, terrestrial), and (E-I) amphibians, birds, fishes, mammals, and reptiles respectively."}
...
kable(tableS1, row.names = FALSE, booktabs = TRUE, longtable = FALSE, caption = "\\textbf{Model coefficients for global population trends}. Median represents the median of the posterior distribution. CI low and high are the lower and higher values of the 95\\% credible interval.** Rhat is the ratio of the effective sample size to the overall number of iterations, with values close to one indicating convergence values.") %>%
...
kable(tableS2, row.names = FALSE, booktabs = TRUE, longtable = FALSE, caption = "\\textbf{Model coefficients for global population trends across systems}. Median represents the median of the posterior distribution. CI low and high are the lower and higher values of the 95\\% credible interval. Rhat is the ratio of the effective sample size to the overall number of iterations, with values close to one indicating convergence values.") %>%
   kable_styling(latex_options = c("repeat_header"), font_size = 7, html_font = "arial")
```
...
kable(tableS6 |> 
        dplyr::mutate(Frequency = round(Frequency,digits = 3)), row.names = FALSE, booktabs = TRUE, longtable = FALSE, caption = "\\textbf{Proportion of threat interaction types across taxa. n represents the number of interaction types present} Frequency represents the proportion of that given interaction type for that threat and taxa.") %>%
   kable_styling(latex_options = c("repeat_header"), font_size = 7, html_font = "arial")
```
...
```{r tableS5b}
kable(tableS5 |> 
        dplyr::mutate(Frequency = round(Frequency,digits = 3)), row.names = FALSE, booktabs = TRUE, longtable = FALSE, caption = "\\textbf{Proportion of threat interaction types across systems. n represents the number of interaction types present}. Frequency represents the proportion of that given interaction type for that threat and system.") %>%
   kable_styling(latex_options = c("repeat_header"), font_size = 7, html_font = "arial")
```
