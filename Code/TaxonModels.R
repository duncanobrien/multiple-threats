...
priors <- c(
  prior_string(paste0("normal(", mean_coef, ",", sd_coef, ")"), class = "b"),
...
spa_mat_trim_amph <- as.matrix(geosphere::distm(amph_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
...
rhs_amph <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_amph)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_amph <- brm(
  bf(form_amph #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
...
spa_mat_trim_bir <- as.matrix(geosphere::distm(bir_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_bir <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_bir)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_bir <- brm(
  bf(form_bir #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_bir,
...
spa_mat_trim_fish <- as.matrix(geosphere::distm(fish_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_fish <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_fish)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_fish <- brm(
  bf(form_fish #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_fish,
...
spa_mat_trim_mam <- as.matrix(geosphere::distm(mam_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_mam <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_mam)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_mam <- brm(
  bf(form_mam #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_mam,
...
spa_mat_trim_rep <- as.matrix(geosphere::distm(rep_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_rep <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_rep)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_rep <- brm(
  bf(form_rep #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_rep,
