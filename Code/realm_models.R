...
mod2_mar <- brm(bf(y_centered ~  scaled_time*none + 
               ...
               data =test_data2 %>% filter(System=="Marine"), 
               family = gaussian(),
               iter = 2000,
               refresh=100,
               #backend = "cmdstanr",
               ...
               cores = 4)
saveRDS(mod2_mar,"Results/models/simple-slopes-marine.RDS")

mod2_fre <- brm(bf(y_centered ~  scaled_time*none + 
                     scaled_time*pollution + scaled_time*habitatl+
                     scaled_time*climatechange + scaled_time*invasive+ 
                     scaled_time*exploitation+scaled_time*disease + 
                     (-1 + time|series) + (1|SpeciesName) - 1
                ...
                data =test_data2 %>% filter(System=="Freshwater"), 
                family = gaussian(),
                iter = 2000,
                refresh=100,
                #backend = "cmdstanr",
                ...
                control=list(adapt_delta=0.975,max_treedepth = 15),
                cores = 4)
saveRDS(mod2_fre,"Results/models/simple-slopes-fresh.RDS")

mod2_ter <- brm(bf(y_centered ~  scaled_time*none + 
                     scaled_time*pollution + scaled_time*habitatl+
                     scaled_time*climatechange + scaled_time*invasive+ 
                     scaled_time*exploitation + scaled_time*disease + 
                     (-1 + time|series) + (1|SpeciesName) - 1
                ...
                data =test_data2 %>% filter(System=="Terrestrial"), 
                family = gaussian(),
                iter = 2000,
                refresh=100,
                #backend = "cmdstanr",
                ...
                control=list(adapt_delta=0.975,max_treedepth = 15),
                cores = 4)
saveRDS(mod2_ter,"Results/models/simple-slopes-terra.RDS")
