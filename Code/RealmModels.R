...
spa_mat_trim_mar <- as.matrix(geosphere::distm(mar_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
...
rhs_mar <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_mar)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_mar <- brm(
  bf(form_mar #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
...
spa_mat_trim_fre <- as.matrix(geosphere::distm(fre_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_fre <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_fre)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_fre <- brm(
  bf(form_fre #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_fre,
...
spa_mat_trim_ter <- as.matrix(geosphere::distm(ter_locs[, c("Longitude", "Latitude")], fun = geosphere::distHaversine)) /
  1000 #km distance between sites
# Normalise the distance
...
rhs_ter <- paste0(
  paste(
    "scaled_year*",
    colnames(mod_dat_ter)[grepl(paste(
      c(
        "pollution",
        "habitatl",
        "climatechange",
...
mod_ter <- brm(
  bf(form_ter #include realm/spp as slopes, x intercepts
     , autocor = ~ ar(
       time = time, gr = series, p = 1
     )),
  data = mod_dat_ter,
